...
class LightboxManager {
    ...
    createShadowContainer() {
        ...
        shadowHost.id = `${this.imageClass}-zoomin-shadow-host`;
  ...
  `;
    ...
    injectStylesToShadowDOM() {
        const styles = `
/* Markdown Lightbox Plugin 样式 - Shadow DOM 版本 */
...
animation: markdownLightboxFadeIn 0.3s ease !important;
  ...
  `;
    ...
    showLightbox(src, alt) {
        ...
        // 启用宿主元素的指针事件
        const shadowHost = document.getElementById(`${this.imageClass}-zoomin-shadow-host`);
        if (shadowHost) {
  ...
  `;
    ...
    handleSVGImage(imgElement) {
        ...
        imgElement.onload = () => {
            ...
            const maxHeight = viewportHeight * 0.85; // 85vh
...
const generateMainStyles = (imageClass = 'markdown-it-image-lightbox-plugin') => {
...
// 图片插件主函数
const actualPluginFactory =  (options = {}) => {
    const {
        lazyLoading = true,
        enableLightbox = true,
...
export default function imageLightboxPlugin(md, options = {}) {
  // 调用工厂函数生成插件，并立即执行
  const plugin = actualPluginFactory(options);
  plugin(md);
}
